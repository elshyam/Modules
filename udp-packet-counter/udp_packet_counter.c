...
static unsigned int UDP_counter_hook_func(const struct nf_hook_ops *ops, 
		                          ...
		                          const struct net_device *out, int (*okfn)(struct sk_buff *))
{
	...
	/*check for UDP packet comparing ip header nnotocol */
...
static int __init udpCounter_init(void)
{
	...
	if (res < 0) {
		printk(KERN_INFO "udp _Packet_counter: error in nf_register_hook()\n");
		return res;
	}
	printk(KERN_INFO "udp_packet_counter loaded\n");
...
static void __exit udpCounter_exit(void)
{
	...
	printk(KERN_INFO "udp_packet_counter unloaded\n");
}
